...
public interface ProcurementRepo extends JpaRepository<Procurement,Long> {
	...
	@Query(value="select count(ipnumber) from procurement where ipstatus='Alloted'",nativeQuery=true)
...
//	@Transactional
	...
	@Query(value="select * from procurement where ipstatus='Alloted'",nativeQuery=true)
	public List<Procurement> findByIpstatus();
}
